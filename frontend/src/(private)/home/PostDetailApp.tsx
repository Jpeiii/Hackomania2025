...
const PostDetailApp = ({ navigation, route }: any) => {
  const { item, user }: { item: IPost; user: IUser } = route.params;
  ...
  return (
    <Block safe margin={sizes.m} padding={sizes.s}>
      <Block scroll keyboard>
        ...
        <Block align="flex-start" justify="space-between">
          <Block row>
        ...
        <Block>
          <Block row>
            ...
            <Text
              textPrimary
              bold
              style={{ marginHorizontal: sizes.s, alignSelf: "flex-end" }}
            >
              {item.location}
            </Text>
          </Block>
          ...
          <Block>
            <Block row paddingVertical={sizes.s}>
              <Text textSecondary semibold>
                Recommendation Level
              </Text>
            </Block>
            <Block row paddingBottom={sizes.s}>
              {[...Array(5)].map((_, i) => (
                <Icon
                  key={i}
                  iconType="AntDesign"
