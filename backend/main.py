...
def connectCrewAI():
    ...
    dermatalogist_task = Tasks().dermatalogist_task(
        determotologist_agent, parsed_json, payload)
    ...
    image = create_image(
        determotologist_parsed_json['predicted_skin_condition_after_meal_plan'], payload.get('image'))
    data = {
